...
def visualize_spectrogram(
    path, duration=None, offset=0, sr=16000, n_mels=128, n_fft=2048, hop_length=512
):
    ...
    for artist in artists:
        ...
        for album in artist_albums:
            ...
            for song in album_songs:
                ...
                if pathlib.Path(os.path.join(save_folder, save_name)).exists():
                    continue
...
def load_dataset_song_split(
    ...
    X_train, X_test, Y_train, Y_test, S_train, S_test = train_test_split(
        X, Y, S, test_size=test_split_size, stratify=Y, random_state=random_state
    )
...
def create_spectrogram_plots(
    artist_folder="artists", sr=16000, n_mels=128, n_fft=2048, hop_length=512
):
...
def predict_artist(
    model, X, Y, S, le, class_names, slices=None, verbose=False, ml_mode=False
):
    """
    ...
    plot_confusion_matrix(
        ...
        classes=class_names,
        normalize=True,
        title="Confusion matrix for pooled results" + " with normalization",
    )
    class_report = classification_report(
        actual_array, prediction_array, target_names=class_names
    )
    print(class_report)

    class_report_dict = classification_report(
        actual_array, prediction_array, target_names=class_names, output_dict=True
    )
...
if __name__ == "__main__":
    ...
    if create_visuals:
        ...
        create_spectrogram_plots(
            artist_folder="artists", sr=16000, n_mels=128, n_fft=2048, hop_length=512
        )
        if save_visuals:
            plt.savefig(os.path.join("spectrograms.png"), bbox_inches="tight")
