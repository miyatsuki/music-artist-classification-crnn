...
def CRNN2D(X_shape, nb_classes):
    ...
    for layer in range(nb_layers - 1):
        # Convolutional layer
        model.add(
            Conv2D(nb_filters[layer + 1], kernel_size=kernel_size, padding="same")
        )
        model.add(Activation(activation))
        model.add(
        ...
        model.add(
            MaxPooling2D(pool_size=pool_size[layer + 1], strides=pool_size[layer + 1])
        )  # Max pooling
        model.add(Dropout(0.1))
...
def CRNN2DLarger(X_shape, nb_classes):
    ...
    kernel_size = (3, 3)  # convolution kernel size
    activation = "elu"  # activation function to use after each layer
    pool_size = [(2, 2), (2, 2), (2, 2), (4, 1), (4, 1)]  # # size of pooling area
    ...
    model.add(MaxPooling2D(pool_size=pool_size[0], strides=pool_size[0]))  # Max pooling
...
def CRNN1D(X_shape, nb_classes):
    ...
    model.add(
        Permute((time_axis, frequency_axis, channel_axis), input_shape=input_shape)
    )
    resize_shape = model.output_shape[2] * model.output_shape[3]
    model.add(Reshape((model.output_shape[1], resize_shape)))
    ...
    model.add(Activation(activation))
    model.add(MaxPooling1D(pool_size=pool_size, strides=pool_size))  # Max pooling
    # model.add(Dropout(0.2))
